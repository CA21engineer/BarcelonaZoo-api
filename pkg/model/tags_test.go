...
func testTagToManyThemeTagRelations(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TagSlice{&a}
	if err = a.L.LoadThemeTagRelations(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTagToManyAddOpThemeTagRelations(t *testing.T) {
	var err error
...
var (
	tagDBTypes = map[string]string{`ID`: `int`, `Text`: `varchar`}
	_          = bytes.MinRead
)
