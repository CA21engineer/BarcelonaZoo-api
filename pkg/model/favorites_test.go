...
func testFavoriteToOneChallengeRecordUsingChallengeRecord(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local Favorite
	var foreign ChallengeRecord

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, favoriteDBTypes, false, favoriteColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Favorite struct: %s", err)
	}
	if err := randomize.Struct(seed, &foreign, challengeRecordDBTypes, false, challengeRecordColumnsWithDefault...); err != nil {
	...
	local.ChallengeRecordID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.ChallengeRecord().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadChallengeRecord(ctx, tx, false, (*[]*Favorite)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ChallengeRecord == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.ChallengeRecord = nil
	if err = local.L.LoadChallengeRecord(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ChallengeRecord == nil {
		t.Error("struct should have been eager loaded")
...
func testFavoriteToOneSetOpChallengeRecordUsingChallengeRecord(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, challengeRecordDBTypes, false, strmangle.SetComplement(challengeRecordPrimaryKeyColumns, challengeRecordColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, challengeRecordDBTypes, false, strmangle.SetComplement(challengeRecordPrimaryKeyColumns, challengeRecordColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*ChallengeRecord{&b, &c} {
		err = a.SetChallengeRecord(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.ChallengeRecord != x {
		...
		if a.ChallengeRecordID != x.ID {
			t.Error("foreign key was wrong value", a.ChallengeRecordID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.ChallengeRecordID))
		reflect.Indirect(reflect.ValueOf(&a.ChallengeRecordID)).Set(zero)
		...
		if a.ChallengeRecordID != x.ID {
			t.Error("foreign key was wrong value", a.ChallengeRecordID, x.ID)
		}
	}
}
...
var (
	favoriteDBTypes = map[string]string{`ID`: `int`, `UserID`: `int`, `ChallengeRecordID`: `int`}
	_               = bytes.MinRead
)
